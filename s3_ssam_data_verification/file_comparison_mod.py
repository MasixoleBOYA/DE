...
#                 # Save differing rows to an Excel file
#                 output_file_name = f"Differing rows between {first_file.replace('.csv', '')} and {other_file.replace('.csv', '')}.xlsx"
#                 output_file_path = os.path.join(folder_path, output_file_name)
#                 differing_rows.to_excel(output_file_path, index=False)
#                 print(f"Differing rows saved to {output_file_path}")
...
def compare_csv_files(folder_path):
    ...
    for other_file in other_files:
        ...
        if base_df.equals(other_df):
            ...
            # Check for new TripNum values
            if "TripNum" in base_df.columns and "TripNum" in other_df.columns:
                base_tripnums = set(base_df["TripNum"].dropna())
                other_tripnums = set(other_df["TripNum"].dropna())

                new_in_other = other_tripnums - base_tripnums
                if new_in_other:
                    print(f"New TripNum values in {other_file} not in {first_file}: {new_in_other}")

                new_in_base = base_tripnums - other_tripnums
                if new_in_base:
                    print(f"New TripNum values in {first_file} not in {other_file}: {new_in_base}")
